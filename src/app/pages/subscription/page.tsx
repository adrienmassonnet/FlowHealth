...
function SubscriptionPageInner() {
  ...
  return (
    <main className="pt-20 bg-white min-h-screen">
      ...
      {/* Hero */}
      <div className="border-b border-[#1E185420]">
      ...
      <div className="max-w-[1200px] mx-auto px-6 py-12 md:py-16">
        <div className="max-w-lg mx-auto">
          ...
          {/* ── Step 1: Email lookup ── */}
          {step === 'lookup' && (
            <div className="bg-white rounded-2xl border border-[#1E185420] shadow-[var(--shadow-card)] p-7 md:p-9">
              <div className="w-10 h-10 rounded-xl bg-[#1E18540A] border border-[#1E185420] flex items-center justify-center mb-5">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3.5 py-2.5 text-sm rounded-xl border border-[#1E185420] bg-[#1E18540A] text-[#1E1854] placeholder:text-[#1E1854]/25 focus:outline-none focus:border-[#1E1854]/40 focus:bg-white transition-colors"
                    />
                  </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white rounded-2xl border border-[#1E185420] shadow-[var(--shadow-card)] p-7 md:p-9">
                ...
                {sub.status === 'paused' && !confirmAction && (
                  <div className="pt-2 border-t border-[#1E185420]">
                    <button
                ...
                {sub.status === 'active' && !confirmAction && (
                  <div className="flex flex-col sm:flex-row gap-2 pt-2 border-t border-[#1E185420]">
                    <button
                      onClick={() => { trackEvent('subscription_pause_click'); ga4SubscriptionEvent('pause_click'); setConfirmAction('pause'); setActionError(''); }}
                      className="flex-1 py-2.5 rounded-xl text-xs tracking-[0.06em] uppercase font-semibold border border-[#1E185420] text-[#1E1854]/70 hover:border-[#1E1854]/40 hover:text-[#1E1854] transition-colors"
                    >
                      Pause subscription
                    </button>
                    <button
                      onClick={() => { trackEvent('subscription_cancel_click'); ga4SubscriptionEvent('cancel_click'); setConfirmAction('cancel'); setActionError(''); }}
                      className="flex-1 py-2.5 rounded-xl text-xs tracking-[0.06em] uppercase font-semibold border border-[#1E185420] text-red-500/70 hover:border-red-200 hover:text-red-600 transition-colors"
                    >
                      Cancel subscription
                    </button>
                  </div>
                ...
                {/* Confirm: pause */}
                {confirmAction === 'pause' && (
                  <div className="pt-4 border-t border-[#1E185420]">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        disabled={actionLoading}
                        className="flex-1 py-2.5 rounded-xl text-xs tracking-[0.06em] uppercase font-semibold border border-[#1E185420] text-[#1E1854]/60 hover:text-[#1E1854] transition-colors disabled:opacity-40"
                      >
                        Keep active
                      </button>
                    </div>
                    ...
                    <div className="space-y-2 mb-4">
                      {CANCEL_REASONS.map((reason) => (
                        <label key={reason} className="flex items-center gap-2.5 cursor-pointer group">
                          <span className={`w-4 h-4 shrink-0 rounded-full border flex items-center justify-center transition-colors ${cancelReason === reason ? 'border-[#1E1854] bg-[#1E1854]' : 'border-[#1E185420] group-hover:border-[#1E1854]/40'}`}>
              ...
              {/* Help nudge */}
              <div className="rounded-2xl border border-[#1E185420] bg-[#1E18540A] px-6 py-5 flex items-start gap-3">
                <span className="shrink-0 mt-0.5 w-8 h-8 rounded-lg bg-white border border-[#1E185420] flex items-center justify-center text-[#1E1854]/50">
...
function ConfirmationCard({
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#1E185420] shadow-[var(--shadow-card)] p-7 md:p-9 text-center">
