...
export default function IngredientCard({ name, form, category, description, dose, imageUrl, imageAlt }: Props) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-ink/[12.5%] overflow-hidden flex flex-row sm:flex-col">
      ...
      <div className="p-3 sm:p-6 flex flex-col gap-2 sm:gap-3 flex-1 min-w-0">
        ...
        <button
          onClick={() => setOpen((v) => !v)}
          className="mt-auto pt-2 sm:pt-3 border-t border-ink/[12.5%] flex items-center justify-between w-full group"
        >
          ...
          <svg
            ...
            className={`text-[rgba(30,24,84,0.4)] transition-transform duration-300 group-hover:text-ink ${open ? 'rotate-180' : ''}`}
          >
