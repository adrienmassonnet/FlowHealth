...
export default async function ReviewsPage() {
  ...
  return (
    <main>
      {/* Hero */}
      <section className="bg-white border-b border-[#1E185420]">
      ...
      {/* Stats bar */}
      <section className="bg-[#1E18540A] border-b border-[#1E185420]">
        <div className="max-w-[1200px] mx-auto px-6 py-5 md:py-10">
          <div className="grid grid-cols-3 gap-3 sm:gap-6 md:gap-0 md:flex md:items-center md:divide-x md:divide-[#1E185420]">
            {stats.map((s) => (
          ...
          {testimonials.length === 0 ? (
            ...
            <div className="columns-1 sm:columns-2 lg:columns-3 gap-3 sm:gap-5 space-y-3 sm:space-y-5">
              {testimonials.map((t, i) => (
                <div
                  key={i}
                  className="break-inside-avoid bg-white rounded-2xl border border-[#1E185420] px-5 py-5 space-y-3 md:px-7 md:py-7 md:space-y-4"
                >
                  <Stars />
      ...
      <section className="bg-[#1E1854]">
        <div className="max-w-[1200px] mx-auto px-6 py-16 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          ...
          <Link
            ...
            className="shrink-0 inline-flex items-center gap-2 bg-white text-[#1E1854] text-xs tracking-[0.1em] uppercase font-semibold px-7 py-4 rounded-full hover:bg-[rgb(30,24,84)] hover:text-white transition-colors"
          >
            Get Flow
          </Link>
        </div>
