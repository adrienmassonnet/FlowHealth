...
function RichTextNode({ node }: { node: any }): React.ReactNode {
  ...
  switch (node.nodeType) {
    case 'document':
      return (
        <div className="space-y-6 text-sm text-[rgba(30,24,84,0.85)] leading-[1.8]">
    ...
    case 'list-item':
      return (
        <li className="flex items-start gap-3">
          <span className="mt-2 shrink-0 w-1.5 h-1.5 rounded-full bg-[rgba(30,24,84,0.6)]" />
          <span>
            {node.content?.map((child: any, i: number) => (
              <RichTextNode key={i} node={child} />
            ))}
  ...
  return (
    <main>
      ...
      <article className="max-w-[720px] mx-auto px-6 py-14">
        ...
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-xs text-[rgba(30,24,84,0.45)] mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          {(post.tags ?? []).map((tag) => (
            <span key={tag} className="text-xs uppercase tracking-[0.1em] border border-[#1E185420] px-3 py-1 rounded-full text-[rgba(30,24,84,0.55)] font-medium">
              {tag}
            </span>
          ))}
          <span className="text-xs text-[rgba(30,24,84,0.4)]">{post.publishedDate} · {post.readTime}</span>
        </div>
        ...
        <p className="text-lg text-[rgba(30,24,84,0.7)] leading-relaxed mb-10 border-l-2 border-[rgba(30,24,84,0.4)] pl-4">
          {post.excerpt}
        </p>
        ...
        {/* Back link */}
        <div className="mt-16 pt-10 border-t border-[#1E185420]">
          <Link
            href="/pages/blog-posts"
            className="inline-flex items-center gap-2 text-xs tracking-[0.08em] uppercase font-medium text-[rgba(30,24,84,0.6)] hover:text-[#1E1854] transition-colors"
          >
      ...
      {otherPosts.length > 0 && (
        <section className="max-w-[1200px] mx-auto px-6 pb-24">
          <div className="border-t border-[#1E185420] pt-16">
              ...
              {otherPosts.map((p) => (
                ...
                >
                  ...
                  <div className="p-5 space-y-2">
                    <p className="text-xs text-[rgba(30,24,84,0.45)]">{p.publishedDate} · {p.readTime}</p>
                    <h3 className="text-sm font-semibold text-[#1E1854] leading-snug group-hover:text-[rgb(30,24,84)] transition-colors line-clamp-2">
                      {p.title}
                    </h3>
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {(p.tags ?? []).map((tag) => (
                        <span key={tag} className="text-xs uppercase tracking-[0.08em] border border-[#1E185420] px-2 py-0.5 rounded-full font-semibold bg-gradient-to-r from-[#3B38B8] to-[#1E1854] bg-clip-text text-transparent">
                          {tag}
                        </span>
                      ))}
                    </div>
