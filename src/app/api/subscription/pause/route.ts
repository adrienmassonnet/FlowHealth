import { pauseSubscription, getSubscriptionByEmail } from '@/lib/shopify-subscriptions';
...
export async function POST(req: Request) {
  try {
    ...
    const subscription = await pauseSubscription(current.id);

    await trackKlaviyoEvent(email, 'Subscription Paused', {
    ...
    return Response.json({ error: 'Failed to pause subscription. Please contact support.' }, { status: 500 });
  }
}
