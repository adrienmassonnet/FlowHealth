...
export default function HealthCenter({ caloriesKcal, activeIngredients, totalFormulaWeightG, servingsPerBox, handle }: { caloriesKcal: number; activeIngredients: number; totalFormulaWeightG: number; servingsPerBox: number; handle: string }) {
  ...
  return (
    <section className="max-w-[1200px] mx-auto px-6 pt-4 pb-20 md:pt-8">
      <div className="mb-5">
        <h2 className="text-3xl font-semibold tracking-[-0.02em]">Our product has nothing to hide.</h2>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <div key={row.label} className="bg-gradient-to-br from-[#1E1854] to-[#2d2a7a] rounded-xl px-4 py-3">
                  <p className="text-lg font-semibold tracking-[-0.02em] text-white leading-snug">{row.value}</p>
                  <p className="text-xs tracking-[0.08em] uppercase text-white/50 mt-1">{row.label}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <div key={label} className="inline-flex items-center px-3 py-2.5 rounded-xl border border-[var(--color-border)] bg-[#1E185403]">
                  <span className="text-xs font-medium text-[#1E1854] leading-snug whitespace-nowrap">{label}</span>
                </div>
              ))}
            </div>
            <button
        ...
        <div className="space-y-4">
          ...
          <div className="border border-[var(--color-border)] rounded-2xl overflow-hidden">
            {pillars.map((p, i, arr) => (
              <div key={p.title} className={`px-4 py-3 bg-[#1E185403] ${i < arr.length - 1 ? 'border-b border-[var(--color-border)]' : ''}`}>
                <p className="text-sm font-medium text-[#1E1854] leading-snug">{p.title}</p>
