          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col overflow-hidden">
            <div className="flex items-center justify-between px-6 py-5 border-b border-ink/[12.5%] shrink-0">
              <div>
                ...
                <h3 className="text-lg font-semibold text-ink tracking-[-0.01em] mt-0.5">Full Nutrition Sheet</h3>
              </div>
              <button
                onClick={() => setNutritionOpen(false)}
                className="w-8 h-8 rounded-full bg-ink/[3.1%] flex items-center justify-center hover:bg-ink/[5.1%] transition-colors shrink-0"
              >
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                  <path d="M2 2L10 10M10 2L2 10" stroke="var(--color-ink)" strokeWidth="1.5" strokeLinecap="round"/>
                </svg>
              </button>
            </div>
            <div className="overflow-y-auto flex-1 px-6 py-6 space-y-8">
              <div>
                ...
                {macros.map((row) => (
                  <div
                    key={row.label}
                    className={`flex items-center justify-between py-2.5 border-b border-ink/[12.5%] ${row.indent ? 'pl-5' : ''}`}
                  >
                    <span className={`text-sm ${row.indent ? 'text-[rgba(30,24,84,0.55)]' : 'text-ink font-medium'}`}>{row.label}</span>
              ...
              <div>
                ...
                {actives.map((a) => (
                  <div key={a.name} className="flex items-center justify-between py-2.5 border-b border-ink/[12.5%]">
                    <span className="text-sm text-ink">{a.name}</span>
                    <span className="text-sm font-semibold text-ink shrink-0 ml-6">{a.dose}</span>
                  </div>
                ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-start">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold tracking-[0.12em] uppercase text-ink/40">Formula</p>
          ...
          <div className="space-y-2">
            <p className="text-xs font-semibold tracking-[0.12em] uppercase text-ink/40">What&apos;s not in it</p>
            <div className="flex flex-wrap gap-1.5">
              {[
                ...
                <div key={label} className="inline-flex items-center px-2.5 py-1 rounded-lg border border-ink/[12.5%] bg-ink/[1.2%]">
        ...
        <div className="space-y-2">
          ...
          <div className="border border-ink/[12.5%] rounded-xl overflow-hidden">
            {pillars.map((p, i, arr) => (
              <div key={p.title} className={`px-3 py-2.5 bg-ink/[1.2%] ${i < arr.length - 1 ? 'border-b border-ink/[12.5%]' : ''}`}>
                <p className="text-xs font-medium text-ink leading-snug">{p.title}</p>
                <p className="text-xs text-ink/45 leading-snug mt-0.5">{p.description}</p>
              </div>
            ))}
          </div>
        </div>
