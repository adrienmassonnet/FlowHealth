    ...
    <section className="max-w-[1200px] mx-auto px-6 pt-4 pb-20 md:pt-8">
      ...
      >
        ...
        <div className="flex-1 space-y-1.5">
          ...
          <span className="text-2xl md:text-4xl font-semibold tracking-[-0.03em] text-[#1E1854]">CHF {flowPrice}<span className="text-sm font-medium text-[#1E1854]/40 ml-1">/mo</span></span>
          <p className="text-xs text-[#1E1854]/45 leading-[1.55]">
        ...
        <div className="flex flex-col gap-1 sm:w-52 shrink-0">
          <p className="text-xs font-semibold tracking-[0.1em] uppercase text-[#1E1854]/35">Per day</p>
          <p className="text-xl md:text-3xl font-semibold tracking-[-0.02em] text-[#1E1854]">CHF {pricePerServing}</p>
          <p className="text-xs text-[#1E1854]/50">That's less than</p>
          <p className="text-xs font-semibold text-[#1E1854]">{ALTERNATIVES[altIndex]}</p>
          <button
            onClick={flipAlternative}
            className="self-start inline-flex items-center gap-1.5 text-xs font-medium text-[#1E1854]/40 hover:text-[#1E1854] transition-colors duration-200 mt-0.5"
          >
            See another
      ...
      <div className="md:hidden flex gap-0 rounded-2xl border border-[#1E1854]/[0.07] overflow-hidden shadow-[0_2px_12px_rgba(30,24,84,0.06)]">
        {/* Left: topic list */}
        <div className="w-[42%] shrink-0 divide-y divide-[#1E1854]/[0.07] border-r border-[#1E1854]/[0.07]">
          {comparisonRows.map((row) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left px-3 py-3 transition-colors duration-200 ${isActive ? 'bg-[#1E1854] ' : 'bg-white hover:bg-[#1E1854]/4'}`}
              >
                <span className={`text-[11px] font-semibold leading-snug ${isActive ? 'text-white' : 'text-[#1E1854]/50'}`}>
                  {row.topic?.[0]}
                </span>
              </button>
            );
          ...
          {comparisonRows.filter(r => r.order === activeRow).map((row) => (
            <div key={row.order} className="space-y-2">
              <div className="flex items-start gap-2">
                <CheckIcon />
                <span className="text-xs text-[#1E1854] leading-[1.55] pt-0.5">
                  {row.feature.replace(/\{active_ingredients\}/g, String(activeIngredients))}
                </span>
              </div>
              <div className="flex items-start gap-2">
                <CrossIcon />
                <span className="text-xs text-[#1E1854]/45 leading-[1.55] pt-0.5">{row.othersLabel}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center" onClick={() => setOpen(false)}>
          ...
          >
            ...
            <div className="overflow-y-auto flex-1 px-6 divide-y divide-[#1E185420]">
              {supplements.map((s) => (
